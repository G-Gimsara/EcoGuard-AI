...
const QRAccessStrip: React.FC = () => (
  ...
  <p className="text-center text-[11px] text-gray-400 mt-2 font-mono">
    ...
    <div className="flex-1 min-w-0">
      ...
      <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400 font-mono">
      ...
      Scan this QR code to access a smart system with  real-time alerts, self-registration, and chatbot-assisted insights.
      </p>
      ...
      <div className="mt-5 text-sm text-gray-500">
        💡 Provides air quality status + health recommendations based on sensor data
      </div>
    </div>
  </div>
);
...
const SensorCard: React.FC<SensorCardProps> = ({ icon, title, value, unit, status }) => {
  const theme = statusTheme(status);
  return (
    <div className={`rounded-2xl border-2 p-5 shadow-sm transition-all duration-300 relative overflow-hidden ${theme.card}`}>
      <div className={`absolute top-0 right-0 w-16 h-16 rounded-bl-full opacity-10 ${theme.dot}`} />
      <div className="flex justify-between items-start mb-4">
        <div className="text-gray-600">{icon}</div>
        {status && (
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full tracking-wide ${theme.badge}`}>
            {status}
          </span>
        )}
      </div>
      <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-500 font-mono mb-2">
        {title} 
      </p>
...
const ChartCard: React.FC<ChartCardProps> = ({ title, unit, data, dataKey, status }) => {
 const theme = {
  chart: "#3b82f6",
  chartBg: "rgba(37,99,235,0.08)",
  dot: "bg-blue-500",
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-xs font-semibold text-gray-800 uppercase tracking-widest font-mono">
          {title}
      ...
      {data.length === 0 ? (
        <div className="flex items-center justify-center h-[200px] text-sm font-mono text-gray-400">
...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/air/dust")
      .then((r) => r.json())
      .then((d) => { setLatestDust(d[0]); setDustH(d); });

    fetch("http://localhost:5000/api/air/gas")
      .then((r) => r.json())
      .then((d) => { setLatestGas(d[0]); setGasH(d); });

    fetch("http://localhost:5000/api/air/air-quality")
      .then((r) => r.json())
      .then((d) => { setLatestTemp(d[0]); setTempH(d); });

    fetch("http://localhost:5000/api/air/co")
      .then((r) => r.json())
      .then((d) => { setLatestCO(d[0]); setCoH(d); });

    fetch("http://localhost:5000/api/air/co2")
      .then((r) => r.json())
      .then((d) => { setLatestCO2(d[0]); setCo2H(d); });
  }, []);
